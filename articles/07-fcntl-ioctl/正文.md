...
程序对普通文件用 `fcntl` 做两件事：读取并打开 `O_APPEND` 状态，再用 `F_DUPFD_CLOEXEC` 复制一个带 close-on-exec 的描述符。随后创建管道，写入 3 个字节，用管道支持的 `FIONREAD` 命令通过 `ioctl` 查询当前可读字节数。
...
环境是 macOS 14.8.8、Apple clang 16.0.0。完整代码在 `fcntl_ioctl_demo.c`：
